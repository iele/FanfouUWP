...
namespace Coding4Fun.Toolkit.Test.WindowsPhone.Samples.Buttons
{
	...
	{
		static ImageSource AddIcon = new BitmapImage(new Uri("/Media/icons/appbar.add.rest.png", UriKind.RelativeOrAbsolute));
		static ImageSource RepeatIcon = new BitmapImage(new Uri("/Media/icons/appbar.repeat.png", UriKind.RelativeOrAbsolute));
		...
		public RoundToggleButtons()
		{
			...
			TogglingContentClick(null, null);
		}

		public ImageSource TogglingImageSource
		{
			get { return (ImageSource)GetValue(TogglingImageSourceProperty); }
			set { SetValue(TogglingImageSourceProperty, value); }
		}

		// Using a DependencyProperty as the backing store for RoundButtonImageSource.  This enables animation, styling, binding, etc...
		public static readonly DependencyProperty TogglingImageSourceProperty =
			DependencyProperty.Register("TogglingImageSource", typeof(ImageSource), typeof(RoundToggleButtons), new PropertyMetadata(AddIcon));
		...
		private void TogglingImageSourceClick(object sender, RoutedEventArgs e)
		{
			TogglingImageSource = (TogglingImageSource != AddIcon) ? AddIcon : RepeatIcon;
		}
